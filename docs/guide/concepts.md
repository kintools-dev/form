...
| `error`          | `string \| null`               | This node's own validator message (from `validators`/`asyncValidator`) if it has one, otherwise its slice of a [`schemaErrorMap`](/form/guide/schema-validation) distributed from the nearest schema-validated ancestor. `null` if neither is set. The one property to read for display. |
| `schemaErrorMap` | `Partial<Record<...>> \| null` | A flat, dot-joined path -> message map produced by this node's own [`schemaValidator`](/form/guide/schema-validation), if any.<br/>A child reads its slice via its own `error`.                                                                                                          |
| `invalid`        | `boolean`                      | `true` if `error` is set. Also `true` if any registered child is invalid, or this node's own [`schemaErrorMap`](/form/guide/schema-validation) is non-empty.                                                                                                                             |
